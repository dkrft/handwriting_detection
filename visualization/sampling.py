...
This module provides tools for selecting samples from an image and making predictions on these samples. The resulting
 prediction are used by the heat map module to create heat maps.
"""
...
class RandomSampler(Sampler):
    ...
    def sample(self, image, predictor, label_aggregator):
        ...
        for i in range(0, len(coordinates)):
            ...
            predictions[(y, x)] = label_aggregator(
                predictor.predict([padded_img[y:y + sample_size, x:x + sample_size]])[0])

        return predictions


class RandomGridSampler(Sampler):
